...
- Added dedicated modifier hotkeys (`[Ctrl+S]` Model Switcher, `[Ctrl+K]` Compact, `[Ctrl+P]` Parameters, `[Ctrl+Y]` Copy) preventing typing collisions with chat messages.
- Added slash commands in Chat input (`/compact`, `/params`, `/model`, `/clear`, `/help`).
- Restricted Chat Playground strictly to GGUF model runtimes (filtering out non-chat ONNX engines).

### Fixed
- Fixed llama.cpp server launch failure caused by bare `--flash-attn` flag by providing explicit `--flash-attn on` argument expected by modern llama-server builds.
- Cleaned up redundant manual task cycling (`[E] Task`) from model browser: models are self-describing via their intrinsic GGUF/ONNX formats and execute automatically under their dedicated engines.
